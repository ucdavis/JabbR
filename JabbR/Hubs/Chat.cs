...
namespace JabbR
{
    public class Chat : Hub, IDisconnect, INotificationService
    {
        ...
        public bool Join()
        {
            ...
            // Get the user from the client state or add new user for first-timers
            ChatUser user = _repository.GetUserByIdentity(currentUser) ?? _service.AddUser(currentUser);
        ...
        public object GetCommands()
        {
            return new[] {
                ...
                new { Name = "build", Description = "Type /build [show|run] [id]. Just /build will show list of all build configs" },
